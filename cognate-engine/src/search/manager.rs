...
use crate::storage::{NotebookManager, NoteMetadata};
...
impl SearchIndexManager {
    ...
    pub fn cache_upsert(&mut self, rel_path: &str, content: &str, modified_time: Option<SystemTime>) {
    ...
    pub fn cache_remove(&mut self, rel_path: &str) {
        ...
        self.notes_cache.retain(|path, _| path != rel_path && !path.starts_with(&prefix));
    }
    ...
    pub async fn search(
        ...
        self.notes_cache.retain(|rel_path, _| note_paths.contains(rel_path.as_str()));
        ...
        for note in notes {
            ...
            let content_match = self.notes_cache
                .get(&note.rel_path)
                .and_then(|indexed| {
