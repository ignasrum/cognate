...
impl NotebookEngineState {
    ...
    pub fn load_from_bytes(bytes: &[u8]) -> Result<Self, EngineError> {
        serde_json::from_slice(bytes)
            .map_err(|err| EngineError::Deserialization(err.to_string()))
    }
    ...
    pub fn process_document(&mut self, path: &str, content: &str, labels: &[String], last_updated: Option<String>) {
        self.search_index.index_document(path, content, labels, last_updated);
...
mod tests {
    ...
    fn test_serialization_cycle() {
        ...
        assert!(restored.search_index.documents.contains_key("recipes/apple-pie"));
    }
    ...
    fn test_tasks_and_metrics() {
        ...
        let task_1 = pending.iter().find(|t| t.text.contains("Finish search")).unwrap();
        assert_eq!(task_1.due_date, Some("2026-07-06".to_string()));
        assert_eq!(task_1.is_completed, false);
        
        let task_2 = pending.iter().find(|t| t.text.contains("Write progress")).unwrap();
        assert_eq!(task_2.due_date, Some("2026-07-10".to_string()));

        // Check document metrics
        let metrics = state.metrics_register.metrics_by_note.get("todos/work").unwrap();
