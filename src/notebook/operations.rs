...
pub async fn create_new_note(
    ...
    let metadata = manager.create_note(rel_path, notes).into_notebook_err()?;
    ...
    super::search::cache_upsert_search_index_note_content(
        ...
        rel_path,
        "",
        manager.get_note_modified_time(rel_path),
    );
...
pub async fn delete_note(
    ...
    let manager = NotebookManager::new(Path::new(notebook_path));
    manager.delete_note(rel_path, notes).into_notebook_err()?;

    // Update search index cache
    super::search::cache_remove_search_index_entries(notebook_path, rel_path);
...
pub async fn move_note(
    ...
    let manager = NotebookManager::new(Path::new(notebook_path));
    let path = manager.move_note(current_rel_path, new_rel_path, notes).into_notebook_err()?;
